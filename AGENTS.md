...
| `voice_barge_in.py` | Speaks a reply via `voice_tts.speak` while concurrently monitoring the mic for the user talking over it (§V.12); stops playback and returns a `BargeInResult` (the interrupting utterance's `Transcript`, plus the chunk playback stopped on) if so, or `None` if the reply finished uninterrupted. A false trigger or dismissal phrase ("never mind"/"continue"/etc.) resumes the reply from that same chunk instead of losing it. Called by `daemon.py`'s `_run_voice_exchange`/`_run_voice_turn` everywhere a voice reply is spoken. |
